...
namespace GameNews.Logic
{
    public class Account
    {
        ...
        {
            return GameNews.DataAccess.AccountDAO.activeWriter(username, bit);
        }

        public static int updateImageOfavaterAccount(string imagename, string username)
        {
            return GameNews.DataAccess.AccountDAO.updateImageOfavaterAccount(imagename, username);
        }

        public static int updatePasswordAccount(string username, string password)
        {
            return GameNews.DataAccess.AccountDAO.updatePasswordAccount(username, password);
        }

        public static Account getAccountByUsername(string username)
        {
            Account account = new Account();
            DataTable dt = GameNews.DataAccess.AccountDAO.getAccountByUsername(username);
        ...
        {
            return GameNews.DataAccess.AccountDAO.deleteAccByUsername(username);
        }
    }
    ...
    class AccountList
    {
        public static List<Account> getAllAccount()
        {
            ...
            DataTable dt = GameNews.DataAccess.AccountDAO.getAllAccount();
            foreach(DataRow dr in dt.Rows)
            {
                Account account = new Account(
                    dr["username"].ToString(),
        ...
        {
            List<Account> accounts = new List<Account>();
            DataTable dt = GameNews.DataAccess.AccountDAO.getAllAccountByNameAndIsWriter( username,  isWriter);
            foreach (DataRow dr in dt.Rows)
            {
                Account account = new Account(
                    dr["username"].ToString(),
